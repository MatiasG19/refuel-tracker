<template>
  <div
    v-if="featureTourStore.vehiclePage && featureTourStore.vehiclePageSteps[0]"
    class="fixed-center item0"
  >
    <div>
      Here are all created vehicles.
      <span class="text-positive">Select</span> a vehicle by clicking on it to
      view its data.
    </div>
  ...
  <div
    v-if="featureTourStore.vehiclePage && featureTourStore.vehiclePageSteps[1]"
    class="fixed-center item1"
  >
    <div>
      Use the the "plus" button in the bottom navigation bar to
      <span class="text-positive">add</span> more vehicles.
    </div>
    <q-btn no-caps size="sm" color="positive" @click="nextStep(2)">Next</q-btn>
  </div>

  <div
    v-if="featureTourStore.vehiclePage && featureTourStore.vehiclePageSteps[2]"
    class="fixed-center item2"
  >
    <div>
      To <span class="text-positive">edit</span> a vehicle, use the "three dot"
      button.
    </div>
